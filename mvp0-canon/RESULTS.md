...
## MVP 3 — Una colección DE COMPONENTES: `Tag` + `InputTags` (rama `claude/input-tags-mvp-dho7z1`)

> Ver `METRICS.md` para el detalle completo (investigación en Zag.js/Ark UI/Park UI, diseño, el hallazgo de reactividad más serio de toda la serie, falsación, verificación en navegador real); este apartado consolida el veredicto. Los 4 componentes anteriores (`actionButton`, `statusChip`, `notificationButton`, `counter`) no se tocan — la validación se hace con DOS componentes nuevos: `tag` (texto + botón de borrar que emite su propio evento `REMOVE`) y `inputTags`, que lo COMPONE — un campo de texto donde Enter añade un tag, y N instancias reales y separadas de `Tag`, una por elemento del array, cada una emitiendo su propio evento que `inputTags` reenvía como el suyo propio, con un nombre distinto (`REMOVE_TAG`) y el índice de qué instancia lo disparó.

### La pregunta que se puso a prueba (revisada tras la primera vuelta)

Una primera versión de esta MVP modeló el tag como un TEMPLATE inline (`items: AnatomyItemPart[]`) dentro de la propia spec de `inputTags` — funcionaba, pero no ponía a prueba nada sobre COMPOSICIÓN de componentes, solo sobre repetición de DOM. La pregunta real, pedida explícitamente en revisión, es distinta y más dura: **¿puede el canon repetir una parte de la anatomía que es una instancia de OTRO componente ya definido — no una plantilla nativa — de modo que cada instancia sea una máquina de Zag separada, con sus propios eventos, y el padre reaccione a ellos bajo un nombre de evento distinto al que el hijo declara?** Esto obligó a rediseñar `repeatOver` para que solo aplique sobre una parte `component` (ya no sobre `element` + `items`), y a extender `slotFill`/`onChildEvent` (de MVP 1) para que funcionen dentro de un bucle.

### Las cuatro capacidades pedidas, y cómo se validó cada una

| # | Capacidad | Cómo se validó | Resultado |
|---|---|---|---|
| 1 | Un componente propio, componible de forma independiente | `tag.spec.json`: `label` (contentSlot) + `deleteTrigger` (contentSlot, `onClick: "REMOVE"`) | ✅ `Tag` se renderiza y testea solo, sin `InputTags` (`test/generated/tag-composition.test.tsx`) |
| 2 | Repetir una parte `component` una vez por elemento del array, no una plantilla nativa | `AnatomyPart.repeatOver`, ahora exclusivo de partes `component` | ✅ N instancias reales de `<Tag>`, cada una su propio `useMachine` |
| 3 | Alimentar cada instancia con el elemento del bucle | `SlotFillValue` gana el kind `"loopItem"` (solo válido junto a `repeatOver`) | ✅ `slotFill: {label: {kind:"loopItem"}}` |
| 4 | El padre reacciona al evento del hijo bajo SU PROPIO nombre, sabiendo qué instancia lo disparó | `onChildEvent` (de MVP 1) + `onChildEventPayload: {childEvent: {payloadField: "$index"}}`, nuevo | ✅ `Tag`'s `REMOVE` → `InputTags`'s `REMOVE_TAG{index}`, confirmado borrando el tag del MEDIO por índice, en jsdom y en Chromium real |

### El hallazgo más serio de toda la serie (no anticipado, encontrado a mitad de implementación)

Con el diseño de arriba ya generando código plausible, el primer test real (borrar el tag del medio de tres) **vació el array entero salvo el primero**. Aislado con un repro mínimo (`StatusChip` + un botón que fuerza un re-render del padre sin ningún clic nuevo): **el hook `watch` de Zag-js re-invoca `prop("onEvent")?.(event.current())` cada vez que CUALQUIER prop de `useMachine` cambia de identidad — no solo cuando ocurre un evento real** — y `event.current()` sigue devolviendo la MISMA referencia del último evento real mientras no llegue uno nuevo. Como el callback `onEvent` de una parte `repeatOver` se recrea en cada `.map()`, CUALQUIER re-render del padre (tipear en el input, añadir o borrar otro tag) recrea el callback de cada instancia superviviente y hace que Zag **reproduzca su último evento otra vez** — un `REMOVE` viejo, reenviado contra la posición del array que ahora ocupa una tag distinta. Ningún componente anterior lo había disparado porque ninguno tenía un padre cuyo re-render recreara el callback de un hijo ya usado. Arreglado en el compilador, no en la spec: `emit/component.ts` ahora declara un `WeakSet` a nivel de módulo (`FORWARDED_CHILD_EVENTS`) y descarta cualquier evento ya reenviado por identidad de objeto — corrige también un bug latente, nunca disparado, en `notificationButton`/`statusChip`.

Segundo hallazgo, encontrado depurando el primero: `watch` tampoco se re-invoca para un evento real si la transición es un self-loop que no cambia de estado (`active --REMOVE--> active`, como en `Counter`). `Tag` necesitó dos estados (`active` → `removed`, terminal) — no porque el diseño lo pidiera, sino porque es la única forma de que `watch` note que "pasó algo" y reenvíe.

### Falsación (8 mutaciones reales contra `input-tags.spec.json`, cada una revertida)

| # | Mutación | Resultado |
|---|---|---|
| 1 | `repeatOver` sobre una parte nativa (sin `component`) | Rechazado por el esquema Zod (3 issues a la vez: `slotFill`, `onChildEvent`, `repeatOver`) |
| 2 | `onChildEventPayload` sin `onChildEvent` | Rechazado por el esquema Zod |
| 3 | `onChildEventPayload` para un evento que `onChildEvent` no reenvía | Rechazado por el esquema Zod |
| 4 | `slotFill` con `loopItem` en una parte sin `repeatOver` | Rechazado por el esquema Zod |
| 5 | `onChildEventPayload` rellena un campo que el evento propio no declara | Rechazado — `child-event-payload-exists` |
| 6 | `onChildEventPayload` alimenta un campo `number` con `$item` (produce `string`) | Rechazado — `child-event-payload-exists`, "types don't match" |
| 7 | `slotFill` a una clave que `tag` no declara como slot | Rechazado — `slot-fill-unknown-key` (regla de MVP 1, reutilizada sin cambios) |
| 8 | `slotFill` no rellena un contentSlot requerido de `tag` | Rechazado — `slot-fill-missing-required` (regla de MVP 1, reutilizada sin cambios) |

### Qué no cupo (resumen — detalle completo en `METRICS.md`)

Un solo nivel de repetición (un `repeatOver` no puede anidar otro); ningún atajo de teclado sobre el array salvo Enter-para-añadir (sin Backspace-borra-el-último, que exigiría leer el DOM efímero del input como guarda); el trim + no-vacío de `submitOnEnter` sigue siendo una convención fija del compilador, no un dato de la spec; y el generador de tests/stories sigue sin vocabulario para el VALOR del contexto (mismo límite de MVP 2), así que la mutación real del array solo queda probada a mano (`input-tags-composition.test.tsx`, con un caso que estresa deliberadamente el hallazgo del `WeakSet`: borrar, añadir, borrar otra vez, en secuencia).

### Veredicto

**Go**, con una corrección real de por medio. Las cuatro capacidades funcionan end-to-end — 22/22 tests nuevos en `verify.test.ts`, 180/180 en todo el repo, `Tag` e `InputTags` compilando/testeando/buildeando junto a los 4 componentes existentes sin tocarlos, 8 mutaciones reales rechazadas, y verificación en Chromium real bajo estrés (añadir 4, borrar del medio, añadir otro, borrar el primero) — pero el coste no estuvo en el diseño del schema (limpio: `repeatOver` ahora vive exclusivamente en partes `component`, reutilizando `slotFill`/`onChildEvent` de MVP 1 con dos extensiones pequeñas) sino en una interacción de reactividad de la librería de terceros que ningún test unitario del compiler podía haber predicho — solo generar el componente real, componerlo con otro real, y hacerlo re-renderizar bajo condiciones reales lo sacó a la luz. Es la prueba más fuerte hasta ahora de por qué este proyecto insiste en probar contra el árbol generado de verdad, no solo contra el compilador que lo produce.
